...
//                    }
    ...
    public void writeInfo(List<Map.Entry<String, Integer>> list, String recodePath, String recodeName) {
        ...
        try {
            ...
            fos = new FileOutputStream(new File(recodePath + "\\recode.txt"));
            fos.write("fileStatus\n".getBytes());
            fos.write("文件类型\t\t\t个数\n".getBytes());
            for (Map.Entry<String, Integer> mapping : list) {
                fos.write(mapping.getKey().getBytes());
                if(mapping.getKey().getBytes().length==3){
                    fos.write("\t".getBytes());
                }
                fos.write("\t\t\t".getBytes());
                fos.write(mapping.getValue().toString().getBytes());
                fos.write("\n".getBytes());
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static void main(String[] args) {
        ...
        FileOperationDemo f = new FileOperationDemo();
